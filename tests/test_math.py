...
def test_ensure_symmetric() -> None:
    ...
    assert np.allclose(A_sym, np.array([[1.0, 2.5], [2.5, 4.0]]))
...
def test_numerical_gradient() -> None:
    ...
    assert np.allclose(grad, np.array([4.0, 9.0]), atol=1e-5)
...
def test_regularize_covariance() -> None:
    ...
    assert np.all(eigvals >= 0.1 - 1e-10)  # Allow for numerical tolerance
